...
function LaunchCardPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/products/${targetId}`)
      .then((res) => {
    ...
    <div className="mx-auto max-w-7xl">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.2fr_1.8fr]">
        ...
        <div className="space-y-6">
          <Section title="AI Generated Media" description="FLUX.1 Dev studio photography & Wan 2.1 cinematic ad.">
            <div className="space-y-4">
              {/* Generated Product Image from FLUX.1 Dev */}
              {(() => {
                const genImg = productData?.uploads?.find((u: any) => u.file_type === "image" && u.file_path?.includes("flux_product"));
                const imgUrl = genImg ? `${API_BASE}/${genImg.file_path}` : null;
                return imgUrl ? (
                  <div className="relative overflow-hidden rounded-xl border shadow-inner group bg-white aspect-square flex items-center justify-center">
                    <img
                      src={imgUrl}
                      alt="FLUX.1 Dev Generated Product"
                      className="w-full h-full object-contain group-hover:scale-[1.01] transition-transform duration-300"
                    />
                    <div className="absolute bottom-4 right-4">
                      <Badge className="bg-indigo-600 hover:bg-indigo-600">FLUX.1 Dev · Generated</Badge>
                    </div>
                  </div>
                ) : (
                  <div className="relative overflow-hidden rounded-xl border aspect-square bg-gradient-to-tr from-emerald-50 to-teal-50 flex items-center justify-center p-8 shadow-inner group">
                    ...
                    <div className="w-48 h-64 rounded-3xl bg-gradient-to-tr from-emerald-100/50 via-teal-50 to-white border-2 border-slate-200/50 shadow-2xl flex flex-col items-center justify-center relative p-6 group-hover:scale-[1.01] transition-transform">
                      ...
                      <div className="w-24 h-24 rounded-full bg-gradient-to-tr from-emerald-500 to-teal-400 opacity-80 blur-sm shadow-inner" />
                      <span className="font-display font-bold text-slate-800 mt-6 text-sm tracking-wide text-center">{title.split(" ").slice(0, 3).join(" ")}</span>
                      <span className="text-[10px] text-emerald-600 font-semibold mt-1">✨ AI GENERATED</span>
                    </div>
                    <div className="absolute bottom-4 right-4"><Badge className="bg-indigo-600 hover:bg-indigo-600">Studio Preset</Badge></div>
                  </div>
                );
              })()}

              {/* Wan 2.1 Video Ad Player */}
              {(() => {
                const genVid = productData?.uploads?.find((u: any) => u.file_type === "video" && u.file_path?.includes("wan21_ad"));
                const vidUrl = genVid ? `${API_BASE}/${genVid.file_path}` : null;
                return vidUrl ? (
                  <div className="rounded-xl border overflow-hidden bg-black">
                    <div className="px-4 pt-3 flex items-center justify-between">
                      <span className="text-xs font-semibold text-white">Wan 2.1 · Cinematic Ad</span>
                      <Badge className="bg-purple-600 text-white text-[10px]">9:16 · 15s</Badge>
                    </div>
                    <video
                      src={vidUrl}
                      controls
                    ...
                    />
                    <div className="px-4 pb-3 text-[10px] text-slate-500 font-mono mt-1">Generated by Wan 2.1 T2V-1.3B · LaunchOps AI</div>
                  </div>
                ) : (
