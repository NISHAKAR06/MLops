...
import { API_BASE, getWsBase } from "@/lib/api-base";
...
function UploadPage() {
  ...
  const handleLaunchPipeline = async () => {
    // Enforce the constraint: At least 3 active input channels must be provided
    const inputChannels = [];
    if (productName.trim() !== "" || shortDescription.trim() !== "") inputChannels.push("Text Prompt");
    if (imageFiles.length > 0) inputChannels.push("Product Photo");
    if (videoFiles.length > 0) inputChannels.push("Demo Video");
    if (audioFiles.length > 0 || recordedUrl !== null) inputChannels.push("Voice Memo");

    if (inputChannels.length < 3) {
      toast.warning(
        `Insufficient input channels. Fused e-commerce product launch requires at least 3 distinct sensory inputs. Provided: ${inputChannels.length > 0 ? inputChannels.join(", ") : "None"}`
      );
      return;
    }
    ...
    setCurrentStep(0);
    setLogs(["[System] Spawning pipeline thread...", `[System] Connecting to WebSocket ${getWsBase()}/ws/pipeline...`]);
    ...
    ws.onmessage = (event) => {
      ...
      if (txt.includes("initializing") || txt.includes("pipeline")) {
        setCurrentStep(0);
      } else if (txt.includes("asr") || txt.includes("whisper")) {
        setCurrentStep(2);
      } else if (txt.includes("yolo") || txt.includes("vision")) {
        setCurrentStep(1);
      } else if (txt.includes("ocr") || txt.includes("trocr")) {
        setCurrentStep(3);
      } else if (txt.includes("fusion")) {
        setCurrentStep(4);
      } else if (txt.includes("classifier") || txt.includes("bert")) {
        setCurrentStep(5);
      } else if (txt.includes("llama") || txt.includes("generative") || txt.includes("copywriting")) {
        setCurrentStep(6);
      } else if (txt.includes("flux") || txt.includes("visuals") || txt.includes("stable diffusion")) {
        setCurrentStep(7);
      } else if (txt.includes("wan") || txt.includes("video")) {
        setCurrentStep(8);
      } else if (txt.includes("committed") || txt.includes("database") || txt.includes("transaction")) {
        setCurrentStep(9);
      }
    };
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(`network_failed: ${errText}`);
      }
      const data = await res.json();

      setCurrentStep(10);
      const sku = data.product_id || "P-DEMO99";
      setLogs((prev) => [
        ...prev,
        `[System] Multimodal synthesis complete. Product successfully registered!`,
        `[Database] Generated listing committed with SKU ID: ${sku}`,
      ]);
      setGeneratedProductId(sku);
